(function (){
    //bsj5 - Adding in client side to display 
            ...
            function getResult() {
                ...
                .then(res => res.json())
                .then(jsn => {displaySportEvents(jsn)})
            }
    //BEACH
    function getSportEvents(){
        fetch(`/holidayEvents/Sport`)
        .then (res => res.json())
        .then (jsn => displaySportEvents( jsn))
    }
    ...
    var pCount = 0
    function displaySportEvents(sports){
        pCount = 0
        // console.log(sports)
        let sportsEvents = document.getElementById("sportsEvents");
        sportsEvents.innerHTML = "";
        for(sport of sports) {
            let idSport = sport._id
            let div = document.createElement("div")
            div.value = sport.id
           div.className = "postTest";
            let img = document.createElement("img");
            img.setAttribute("src", sport.src);
            ...
            p1.innerHTML = sport.name 
            p2.innerHTML = sport.rating
            p3.innerHTML = sport.location
            p4.innerHTML = sport.transportation
            let btn1 = document.createElement("button")//ht61 button
            btn1.setAttribute("value", idSport)
