(function () {
    ...
    function getResult() {
        ...
        fetch(`/holidayEvents/Museum/${filRating}/${filTime}`)
            .then(res => res.json())
            .then(jsn => { displayMuseumEvents(jsn) })
    }
    //Museum
    function getMuseumEvents() {
        fetch(`/holidayEvents/Musem`)
            .then(res => res.json())
            .then(jsn => displayMuseumEvents(jsn))
    }

    var pCount = 0
    function displayMuseumEvents(museums) {
        pCount = 0
        // console.log(museums)
        let museumEvents = document.getElementById("museumEvents");
        museumEvents.innerHTML = "";
        for (museum of museums) {
            let idMuseum = museum._id
            let div = document.createElement("div")
            div.value = museum.id
            // div.class = "postTest"
            div.className = "postTest";
            let img = document.createElement("img");
            img.setAttribute("src", museum.src);
            ...
            p1.innerHTML = museum.name
            p2.innerHTML = museum.rating
            p3.innerHTML = museum.location
            p4.innerHTML = museum.transportation
            let btn1 = document.createElement("button")//ht61 button
            btn1.setAttribute("value", idMuseum)
